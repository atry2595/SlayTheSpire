...
BlueSlaverItem::BlueSlaverItem(abstractEntity* source, QPointF pos, qreal zValue)
    :abstractEntityItem(source, pos, {BlueSlaverItem::height, BlueSlaverItem::width + 110}, zValue)
{
    //=======parent=========
    ...
    //=======image========
    entity_image = new ImageItem(entity_parent, {BlueSlaverItem::height, BlueSlaverItem::width}, {0, 50});
    ...
    hp_bar->setSize(BlueSlaverItem::height, 15);

    hp_proxy = new QGraphicsProxyWidget(entity_parent);
    hp_proxy->setWidget(hp_bar);
    hp_proxy->setPos({0, BlueSlaverItem::width + 50});


    //=======anim=========
    auto& anim = entity_parent->attack_animation;
    anim = new QSequentialAnimationGroup(entity_parent);
    anim->addAnimation(entity_parent->createMoveAnimation({pos.x() - BlueSlaverItem::width / 2, pos.y()}, 321, QEasingCurve::InQuart));
...
void BlueSlaverItem::updateEntity() {
    ...
    intent = new TextItem(entity_parent, {45, 45}, {(BlueSlaverItem::width/2 + 17), -8});
    ...
    if (intnt.damage) {
        QString txt = QString::number(12);
        if (intnt.repeat >= 1) {
            txt += "x";
            txt += QString::number(5);
        }
        intent->setText(txt);
    ...
    gr->addAnimation(intent->createMoveAnimation({(BlueSlaverItem::width/2 + 17), -8 -10}, 1500, QEasingCurve::InOutSine));
    gr->addAnimation(intent->createMoveAnimation({(BlueSlaverItem::width/2 + 17), -8}, 1500, QEasingCurve::InOutSine));
    gr->setLoopCount(-1);
    gr->start();
    ...
    for (int i = 0; i < entity_source->get_power_list().size(); i++){
        TextItem* p = new TextItem(entity_parent, {35, 35}, QPointF(35*i, BlueSlaverItem::width + 50 + 20));
